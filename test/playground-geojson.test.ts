...
const fixturesDir = path.resolve(__dirname, 'fixtures/playground-geojson')
...
describe('Playground public data GeoJSON regression', () => {
  it.each([false, true])(
    'matches checked-in fixtures (smooth=%s)',
    async (smooth) => {
      for (const datasetCase of datasetCases) {
        const dataset = await loadDataset(datasetCase)
        const generated = normalizeFeatureCollection(generateGeoJSON(dataset, smooth))

        if (useHash(datasetCase, smooth)) {
          // Hash comparison for large fixtures
          const hashFile = await readFile(fixturePath(datasetCase, smooth), 'utf8')
          const expectedHash = hashFile.trim()
          const actualHash = hashFeatureCollection(generated)
          expect(actualHash).toBe(expectedHash)
        } else {
          // Deep equality comparison for GeoJSON fixtures
          const fixtureContent = await readFile(fixturePath(datasetCase, smooth), 'utf8')
          const fixture = normalizeFeatureCollection(JSON.parse(fixtureContent) as GeoJSON.FeatureCollection)
